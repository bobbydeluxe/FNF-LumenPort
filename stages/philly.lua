...
function onEvent(eventName, value1, value2, strumTime)
	if eventName == 'Philly Glow' then
		...
		elseif value1 == '1' then -- Activates the event, and/or chooses a random color.
			...
			for i, object in ipairs({'boyfriend', 'dad', 'gf'}) do
				setProperty(object..'.color', charactersColors[selectedEventColor])
			end
			...
			if lowQuality == false then
				...
				for y = 1, 3 do
					for x = 1, particlesNum do
						...
						createParticle(-400 + particleWidth * x + offsetX, 425 + (offsetY + y * 40), getColorFromHex(windowsEventColors[selectedEventColor]))
					end
				end
			end
			setGraphicSize('gradient', 2000, 400)
